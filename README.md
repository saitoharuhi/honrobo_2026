...
#### 💡 対話プロンプトによる選択
起動スクリプトを実行すると、ターミナルで以下の選択を求められます。

##### 1. 自己位置（オドメトリ）ソースの選択
*   **1) 外部マイコンを使用する (デフォルト: 1)**: PC側の `zikoiti_node` は起動せず、マイコンから `0x520` IDでCAN送信されてくる自己位置を受信して使用します。
*   **2) PC側ノードを使用する**: PCに接続されたジャイロ（IMU）およびOTOSセンサーから `zikoiti_node` がデータを読み取って自己位置を生成します。

##### 2. 競技エリア・マップ (Map Zone) の選択
*   **1) 赤ゾーン (Red Zone)**: `map_red.yaml` を使用します。
*   **2) 青ゾーン (Blue Zone)**: `map_blue.yaml` を使用します。
*   **3) テストフィールド (Test Field)**: `map_test.yaml` (2.0m×2.0mの狭小エリア) を使用します。このマップを選択した場合、プリセット3および4の目標地点が前後10cm移動に自動設定され、狭い場所での動作テストが可能になります。

#### 起動スクリプトのオプション
| オプション | 説明 |
|-----------|------|
| `--no-can` / `-no-can` | SocketCAN(can0)初期化処理をスキップし、`can_node` を擬似デバッグ用の `Mock Mode` で起動します |
