...
基本的なrosのライブラリが入っていることを前提としています。このプログラムでエラーが出た場合必要なライブラリがない証拠です。各自で調べてインストールしてください。  
...
・can可視化コマンドは必ず実行しなければならないというわけではない。rosのプログラムが完成していない状態でcanが送れているのか確認したい場合に役に立つ。  
...
### 4.1 (ここからは不確定要素が多く含まれる内容です。実行する際はほかの方法を行うことをお勧めします。)
