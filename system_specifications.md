...
## 5. 起動スクリプト (`start_robot.sh` / `start_operator.sh` / `start_all.sh`) の仕様

複数のノードを効率的に管理し、接続ハードウェアの有無に応じたデバッグを可能にするため、以下の起動用スクリプト群を提供します。

### ① 起動オプション
各起動スクリプトは以下の引数（オプション）をサポートします。
*   `--no-can` または `-no-can`
    *   **SocketCANの初期化スキップ**: 起動時の `setup_can.sh`（管理者パスワードの入力）をスキップします。
    *   **`can_node` の擬似モード（Mock）切り替え**: `can_node` ノードの起動パラメータとして `--no-can` が自動的に渡され、実機のCAN接続エラーを回避します。このモードでは送信コマンドがターミナル上で `(MOCKED)` とダミー表示され、実機なしの机上デバッグが可能となります。
*   `--no-build`
    *   **ビルドプロセスのスキップ**: 起動の高速化のため、`colcon build` によるパッケージビルドをスキップします。

### ② マップ選択と動的プリセット設定 (Test Field対応)
ロボット側PC用 (`start_robot.sh`) および一括用 (`start_all.sh`) スクリプトでは、起動時に対話プロンプトでマップゾーン（赤・青・テスト）の選択が可能です。
*   **赤ゾーン / 青ゾーン**: 公式の競技エリアのマップ（`map_red.yaml` / `map_blue.yaml`）を読み込みます。
*   **テストフィールド (Test Field)**:
    *   狭い試験場や机上テストを想定した $2.0\text{m} \times 2.0\text{m}$ 四方の正方形マップ（`map_test.yaml`）を読み込みます。
    *   **プリセット位置のオフセット自動設定**: `web_node.py` の自己位置検知において、読み込まれたマップ名が `map_test.png` である場合、プリセット3およびプリセット4の目標 $(X, Y)$ 座標が自動的に **前後10cm（100mm）の微小移動** に上書き再設定されます。これにより、広いスペースを必要とせずに自動運転の挙動テスト（移動 ➔ アクション実行 ➔ 向き戻しシーケンス）を安全に行えます。
